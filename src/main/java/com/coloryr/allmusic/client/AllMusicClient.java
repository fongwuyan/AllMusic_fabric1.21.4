...
public class AllMusicClient implements ClientModInitializer, AllMusicBridge {
    ...
    public void onInitializeClient() {
        AllMusicDebug.init(FabricLoader.getInstance().getConfigDir());

        DebugCommand.register();
        AllMusicDebug.log("Mod initialized");

        ClientPlayNetworking.registerGlobalReceiver(MusicCodec.ID, (pack, handler) -> {
            try {
                AllMusicDebug.logPacket(pack.pack());
                AllMusicCore.packDo(pack.pack());
            } catch (Exception e) {
                AllMusicDebug.logError("Packet: " + e.getMessage());
                e.printStackTrace();
            }
        });

        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
            AllMusicDebug.markConnect();
        });

        AllMusicCore.init(FabricLoader.getInstance().getConfigDir(), this);
        RenderSystem.recordRenderCall(() -> {
            AllMusicCore.glInit();
            AllMusicDebug.log("Render system ready");
        });
    }
}
