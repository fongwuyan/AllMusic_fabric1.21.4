...
public class AllMusicHud {
    ...
    private final TextFrameBuffer queueRender;
     ...
     * 图片buffer
     */
    private volatile byte[] sourceImage;
    /**
     * 图片buffer
     */
    private volatile byte[] rotateImage;
    //显示信息
    private String info = "";
     ...
     * 是否需要更新材质
     */
    private volatile boolean imageNeedUpload;
    /**
     * 是否需要文字
     */
    ...
    private boolean queueNeedUpdate;
    ...
    public AllMusicHud(int size) {
        ...
        queueRender = AllMusicCore.bridge.makeTextRender("queue");
    ...
    public void clear() {
        haveImg = false;
        info = lyric = lyricTran = lyricKtv = queue = "";
        allTime = nowTime = 0;

        infoNeedUpdate = true;
        lyricNeedUpdate = true;
        stateNeedUpdate = true;
        queueNeedUpdate = true;
    }
    ...
    private void loadPic(String picUrl) {
        ...
        try {
            ...
            while (save == null || imageNeedUpload) {
                if (++waitCount > 250) {
                    AllMusicCore.bridge.sendMessage("图片加载超时");
                    return;
                }
                Thread.sleep(200);
            }
    ...
    public void setPos(HudPosObj save) {
        if (save.queue == null) {
            save.queue = HudPosObj.makeQueue();
        }
        this.save = save;
    ...
    public void update() {
        ...
        if (stateNeedUpdate) {
            if (allTime == 0) {
                ...
                int allHeight = AllMusicCore.bridge.getFontHeight() + (save.state.shadow ? 1 : 0);

                stateRender.resize(allWidth, allHeight);
                stateRender.use();
                stateRender.drawText(all, 0, save.state.color, save.state.shadow);
                stateRender.drawText(time, 0, save.state.color, save.state.shadow);
                stateRender.unUse();
            }
            stateNeedUpdate = false;
        }

        if (queueNeedUpdate && save.queue != null) {
            if (!queue.isEmpty()) {
                int offset = 0;
                String[] temp = queue.split("\n");

                int height = AllMusicCore.bridge.getFontHeight() + (save.queue.shadow ? 1 : 0);
                int allHeight = (height + save.queue.gap) * temp.length;
                int allWidth = 0;
                ...
                queueRender.resize(allWidth, allHeight);
                queueRender.use();
                for (String item : temp) {
                    if (item.isEmpty()) {
                        continue;
                    }
                    queueRender.drawText(item, offset, save.queue.color, save.queue.shadow);
                    offset += save.queue.gap;
                }
                queueRender.unUse();
            } else {
                queueRender.clear();
        ...
        if (save.pic.enable && haveImg) {
            float dispSize = save.pic.size;

            if (save.pic.rotate) {
                bg1.drawPic(save.pic.x, save.pic.y, dispSize, dispSize, save.pic.pos, save.pic.alpha);
            }

            Point2f point = getPos(dispSize, dispSize, save.pic.x, save.pic.y, save.pic.pos);
            picRender.drawAt(save.pic.rotate, point.x, point.y, dispSize, ang, save.pic.alpha);

            if (save.pic.rotate) {
                bg3.drawPic(save.pic.x, save.pic.y, dispSize, dispSize, save.pic.pos, save.pic.alpha);
            }
        }
        //绘制时间
        if (save.state.enable && allTime != 0) {
            int gap = save.state.gap;
            Point2f curr = stateRender.getLine(1);
            Point2f total = stateRender.getLine(0);
            float barW = progress1.width;
            float totalW = curr.x + gap + barW + gap + total.x;
            float fontH = curr.y;
            Point2f point = getPos(totalW, fontH, save.state.x, save.state.y, save.state.pos);

            stateRender.drawLine(point.x, point.y, save.state.alpha, 1);

            float barX = point.x + curr.x + gap;
            progress1.drawPic(barX, point.y + pgOffset, save.state.alpha);
            progress2.drawPic(barX, point.y + pgOffset, (float) nowTime / allTime, save.state.alpha);

            float x1 = barX + (((float) nowTime / allTime) * barW) - ((float) progress3.width / 2);
            progress3.drawPic(x1, point.y + pgOffset, save.state.alpha);

            stateRender.drawLine(point.x + curr.x + gap + barW + gap, point.y, save.state.alpha, 0);
        }
        //绘制队列
        if (save.queue != null && save.queue.enable) {
            queueRender.draw(save.queue.alpha, save.queue.x, save.queue.y, -1, save.queue.pos);
        }
    }
